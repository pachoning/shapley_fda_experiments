...
scenarios_missing = [x for x in os.listdir(output_data_path) if x != ".DS_Store"]
total_scenarios = len(scenarios_missing)

for i_scenario, scenario in enumerate(scenarios_missing):
    msg = f"Working on scenario {i_scenario + 1} out of {total_scenarios}. Id: {scenario}"
    scenario_path = os.path.join(output_data_path, scenario)
    logger.info(msg)
    files_scenario = os.listdir(scenario_path)
    X_sim_file = [x for x in files_scenario if "X_sim_test" in x]
    num_sim_listed = [int(x[11:-4]) for x in X_sim_file]
    num_shapley_files = [int(x[8:-4]) for x in files_scenario if "shapley_" in x]
    num_sim_missing = [x for x in num_sim_listed if not x in num_shapley_files]
    total_sim_missing = len(num_sim_missing)
    for i_sim, num_sim in enumerate(num_sim_missing):
        msg = f"\tWorking on simulation {i_sim + 1} out of {total_sim_missing}. Id: {num_sim}"
        logger.info(msg)
        predict_fn_list = []
        labels_fns_list = []
        all_files = os.listdir(output_data_path)
        # Read the data
        colnames, X_train, X_validation, X_test, target_train, target_validation, target_test = read_data(num_sim, scenario_path)
        ...
        if get_lm_results:
            ...
            r2_test_lm_name = f"r2_test_lm_{num_sim}.pkl"
        ...
        if get_knn_results:
            ...
            r2_test_knn_name = f"r2_test_knn_{num_sim}.pkl"
        ...
        if get_fnn_results:
            ...
            r2_test_fnn_name = f"r2_test_fnn_{num_sim}.pkl"
        ...
        shapley_value_name = f"shapley_{num_sim}.pkl"
